...
describe('instantiate client', () => {
  ...
  describe('defaultHeaders', () => {
    const client = new PropulsionAI({ baseURL: 'http://localhost:5000/', defaultHeaders: { 'X-My-Default-Header': '2' }, bearerToken: 'My Bearer Token' })
  ...
  describe('defaultQuery', () => {
    test('with null query params given', () => {
      const client = new PropulsionAI({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo' }, bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo');
    });

    test('multiple default query params', () => {
      const client = new PropulsionAI({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo', hello: 'world' }, bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo&hello=world');
    });

    test('overriding with `undefined`', () => {
      const client = new PropulsionAI({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' }, bearerToken: 'My Bearer Token' })
      expect(client.buildURL('/foo', { hello: undefined })).toEqual('http://localhost:5000/foo');
    });
  });

  test('custom fetch', async () => {
    const client = new PropulsionAI({ baseURL: 'http://localhost:5000/', bearerToken: 'My Bearer Token', fetch: (url) => {
  ...
  test('custom signal', async () => {
    const client = new PropulsionAI({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', bearerToken: 'My Bearer Token', fetch: (...args) => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new PropulsionAI({ baseURL: 'http://localhost:5000/custom/path/', bearerToken: 'My Bearer Token' });
