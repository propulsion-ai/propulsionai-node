...
export class ChatCompletionStream
  ...
  #endRequest(): CompletionCreateResponse {
    if (this.ended) {
      throw new PropulsionAIError(`stream has ended, this shouldn't happen`);
    }
    const snapshot = this.#currentChatCompletionSnapshot;
    if (!snapshot) {
      throw new PropulsionAIError(`request ended without sending any chunks`);
    }
    this.#currentChatCompletionSnapshot = undefined;
  ...
  protected override async _createChatCompletion(
    ...
    options?: Core.RequestOptions,
  ): Promise<CompletionCreateResponse> {
  ...
  #accumulateChatCompletion(chunk: ChatCompletionChunk): ChatCompletionSnapshot {
    ...
    for (const { delta, finish_reason, index, ...other } of chunk.choices) {
      ...
      if (!choice) {
        choice = snapshot.choices[index] = { finish_reason, index, message: {}, ...other };
      }
      ...
      const { content, role, tool_calls, ...rest } = delta;
...
function finalizeChatCompletion(snapshot: ChatCompletionSnapshot): CompletionCreateResponse {
  ...
  return {
    ...
    id,
    choices: choices.map(
      ({ message, finish_reason, index, ...choiceRest }): CompletionCreateResponse.Choice => {
        if (!finish_reason) throw new PropulsionAIError(`missing finish_reason for choice ${index}`);
        const { content = null, tool_calls, ...messageRest } = message;
        const role = message.role as 'assistant'; // this is what we expect; in theory it could be different which would make our types a slight lie but would be fine.
        if (!role) throw new PropulsionAIError(`missing role for choice ${index}`);
        if (tool_calls) {
          return {
            ...choiceRest,
            index,
        ...
        return {
          ...choiceRest,
          message: { tool_calls: [], ...messageRest, content, role },
          finish_reason,
          index,
        };
      },
...
export namespace ChatCompletionSnapshot {
  export interface Choice {
     ...
     */
    finish_reason: CompletionCreateResponse.Choice['finish_reason'] | null;
